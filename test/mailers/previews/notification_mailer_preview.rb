class NotificationMailerPreview < ActionMailer::Preview
  ...
  def bulk_payment_confirmation_updated_fyi
    submission = FormSubmission.where(role: "bulk_payment").order(id: :desc).first ||
      raise("Need a bulk_payment FormSubmission to preview")

    notification = find_valid_notification("bulk_payment_confirmation_updated_fyi") ||
      Notification.create!(
        noticeable: submission,
        notification_type: 0,
    ...
    NotificationMailer.bulk_payment_confirmation_fyi(notification, updated: true)
  end
