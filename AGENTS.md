...
- **Form submissions index filters + org linking** — the submissions index filters through the shared `events/_filter_bar` shell + `filter_select`/`filter_date` field partials (the shell renders only the fields its caller passes, so no event-specific filter leaks in — those live in `events/_registrant_filters`): person search, form, scenario, `FormSubmission.org_link_status`, `FormSubmission.account_status` — same vocabulary as the registrants roster — and a submitted date range. Each submission with an org answer links to a submission-scoped org-linking editor (`FormSubmissionsController#link_organization`/`select_organization`/`create_organization`), the event-less sibling of the registration one: no submission–org join exists, so "linked" means an active affiliation matches the submitted name and linking runs `SyncProfile`/`UpsertAddress` (fill-blanks) + `AffiliationServices::CreateFromRegistration` (Facilitator affiliation only for purposed forms, dated to the submission). The "Add an organization" block is shared with the registration editor via `shared/_link_organization_actions`
