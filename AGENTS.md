...
- `FeatureCatalog` — Syncs the checked-in feature seed (`config/features.yml`) into the `Feature` table behind the `/features` page (the "Sync latest updates" button). `#import!` creates missing features and fills BLANK fields on existing ones (matched by `name`), never overwriting a field an admin already filled in; returns a `Result` (`created`/`updated`)
