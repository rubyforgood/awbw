...
| `ProfileChangeRequest` | A facilitator's request to correct a profile field only staff can edit (`field`: primary_email / organization_name / affiliation, `requested_value`, free-text `details`). Owner submits from their profile edit page; admins work a "Changes requested" queue (`ProfileChangeRequestsController`, `/profile_change_requests`) — **Approve** auto-applies `AUTO_APPLIABLE_FIELDS` (email via `UserServices::ProcessEmailChange`, org rename) via `ProfileChangeRequests::Apply`, else **Update manually** + **Mark as resolved**, or **Decline**. `status` (pending/resolved/declined) + `resolution_method` (approved/manual). Submission creates admin + submitter `Notification`s (`profile_change_requested`/`_fyi`). Presentation on `ProfileChangeRequestDecorator` |
