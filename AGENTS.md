...
- `ContinuingEducationDeduper` — Collapses duplicate CE registrations left sharing one event registration + license after a merge (there's no DB unique index to make `ModelDeduper` do it, and a blind destroy would drop the duplicate's payments), moving the losers' allocations and comments onto the survivor. Run from the people deduper's `after_merge`
