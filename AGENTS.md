...
- `AffiliationServices::ApplyScenarioEndDating` — What an agreement scenario (`Form::PURPOSES`) means for the person's *existing* affiliations, run by the linking core before creation (see ADR-0002): a **job change** ends their active affiliations at other orgs (job + facilitator alike, sparing the linked org's rows); a **reinstatement** ends any still-active Facilitator affiliations everywhere but leaves job affiliations alone; on-demand/no scenario ends nothing. Ends are dated the day before the agreement takes effect so the fresh affiliations (starting on the submission date) don't overlap — and so `CreateFromRegistration` doesn't skip the reinstatement's fresh Facilitator affiliation as a duplicate of a row still counting as active until end of day
...
- `Form#purpose` (`Form::PURPOSES`) — marks a form as one of the three collaboration agreement intake scenarios (on-demand training, reinstatement, job change), each its own public form so submissions count per scenario. A purposed submission gets an admin **processing panel** on its detail page (`form_submissions/_processing_panel`): submitted organization vs. the person's affiliations (one-click end-dating for a job change via `AffiliationsController#end_affiliation`), plus portal-invite actions; person edit pages grow an "Agreement form links" section that emails the right form's public link (`PeopleController#send_form_link`, recorded as a `form_link_request` notification carrying the form name in `custom_subject` and the public URL in `custom_message`). `FormCopyService` clears `purpose` on copies so two forms never share a scenario
- **Form submissions index filters + org linking** — the submissions index filters through one grid filter bar handled by `FormSubmission.search_by_params` (form, role, event/person/organization remote-selects, submitted date range — plus name/email `search`, agreement `scenario`, `FormSubmission.org_link_status`, and `FormSubmission.account_status`, the same vocabulary as the registrants roster). `FormSubmissionsHelper#form_submission_carryover_params` threads the active filters through a submission's detail page and back. Each submission with an org answer links to a submission-scoped org-linking editor (`FormSubmissionsController#link_organization`/`select_organization`/`create_organization`), the event-less sibling of the registration one: "linked" means an org was explicitly linked to the submission (`FormSubmission#link_organization!`, metadata `linked_organization_ids` — written by this editor **and back-applied by the event registration editor** onto the submission its pinning rules say describes the org, so a typo'd name resolved to a differently-named org still reads as linked) or an active affiliation matches the submitted name. Both editors run the same linking core, **`OrganizationServices::LinkSubmittedOrganization`** (scenario end-dating via `AffiliationServices::ApplyScenarioEndDating`, fill-blanks `SyncProfile`/`UpsertAddress`, `AffiliationServices::CreateFromRegistration` — Facilitator affiliation only for purposed forms/training events — plus `ProfileDiff` conflicts and the flash notice/warning text); the registration flow keeps its own wrapper (link row, pin, autofill notes) and never auto-ends affiliations. **ADR-0002** (`docs/adr/0002-org-linking-flows-and-agreement-scenarios.md`) pins the three linking variants and per-scenario affiliation handling. The "Add an organization" block is shared via `shared/_link_organization_actions`
