...
- `EventDetailsSections` — Splits the event-details ("Art supplies & what to bring") HTML into a lead block plus one collapsible section per `<h3>` heading, so the `events#details` page renders each workshop as its own `<details>` toggle **built in code from the headings** (never authored in the rich text). This keeps the toggles out of reach of the `form_label_html` sanitizer, which strips `<details>`/`<summary>` on save. Shared disclosure markup lives in `app/views/events/callouts/_toggle.html.erb` (also used by the FAQ page); content with no `<h3>` renders unchanged as the lead
