...
- `AttendeesRoster` — Cross-event counterpart to `EventDashboard`: builds the per-registrant lookup maps the shared `events/_registrant_roster` partial reads (sector/age/org/status/location/scholarship/CE plus the attended-trainings event column) for a paginated page of people who have attended a facilitator training; backs the `events#attendees` index. Takes `events:` — the viewer's `EventPolicy` `:reportable` scope — so a person's training columns never surface an event the viewer can't see
- `AttendeesBreakdowns` — Aggregate counterpart to `EventDashboard`'s breakdown methods: computes the chart datasets (sectors, age groups, locations, program status, life experiences, settings, organizations, scholarship/CE) over an arbitrary people set, profile-sourced, for the shared `events/_registrant_breakdowns` partial. Backs the `events#attendees` index charts (cross-event, `events:` = the viewer's reportable trainings, `registrations:` = attended facilitator trainings) and the `events#recipients` charts frame (one event's scholarship recipients, `registrations:` = `EventRegistration.active` so it counts them regardless of attendance)
- `AttendeesActiveFilters` — Human-readable, removable-chip descriptors for the `events#attendees` index's "drill-in" filters (registrant_ids, organization, org city, age group / life experience / setting categories, country, school district, scholarship, CE) — the params that narrow the roster but have no field in the visible filter form; the filter partial surfaces each applied one as a chip so an admin can see and clear it
...
- `RegistrantCityBreakdown` (presenter) — Groups an event's registrants by the city of the org linked on their registration, counting registrants + scholarship recipients per city; drives the shared "Registrants by city" card on the per-event roster page, the cross-event attendees index, and the scholarship-recipients page (fed plucked data by `EventDashboard`)
