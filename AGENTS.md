...
- `PersonServices::ReconcileDefaultPayCustomer` — After a person merge, settles the survivor to a single default Pay customer. Merging two billable people can leave the survivor owning two live `pay_customers` both flagged `default` (the soft-delete unique index doesn't bound them in MySQL, so `ModelDeduper` moves both instead of collapsing), which makes Pay's `payment_processor` (`has_one … default: true`) ambiguous. Keeps the survivor's own pre-merge default (id captured in the people deduper's `merge_keeper`), else the newest, and demotes the rest; no-op unless more than one default remains. Runs in `after_merge`
