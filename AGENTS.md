...
- `EventRegistrationServices::CalloutFormSubmission` — Records a registrant's answers to the form a registration-ticket callout delivers inline (`RegistrationTicketCallout#form`). Find-or-creates one `FormSubmission` per `(registrant, form, event)` — a re-submit edits the answers in place via `FormSubmission#persist_answer`. The submission mirrors the form's own role (`.role_for`, so it looks like any other submission of that form), falling back to `"callout"` when the form is role-less or carries a reserved `EventForm::ROLES` role. Stamps provenance in `FormSubmission#metadata` (`collected_via: "callout"`, `collected_via_callout_id`) via `#record_callout_collection!` — the flag, not the role, is how you tell a callout-collected submission apart. Drives `Events::CalloutsController#callout`/`#submit_callout` (the person-first `registration/:slug/forms/:callout_id` page)
