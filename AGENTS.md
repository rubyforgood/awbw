...
- `CalloutContent` — Parses admin-authored callout HTML into ordered segments so **every** callout content page renders the same way: plain rich text, with each custom `<toggle title="…">…</toggle>` (or `<toggle><summary>…</summary>…</toggle>`) element shown as a styled collapsible section. The `<toggle>`/`<summary>` tags are consumed here, **before** `form_label_html` sanitizes the body, so the disclosure markup (which that allowlist strips) is built in code and can never be lost on save — no need to widen the sanitizer. Rendered through the shared `app/views/events/callouts/_rich_content.html.erb` partial (which wraps each toggle in `_toggle.html.erb`), used by the event-details ("Art supplies & what to bring"), CE hours, custom-callout, behavioural-card-intro, and FAQ pages. The FAQ page renders the editable `faq` callout `description` (each question a `<toggle>`), falling back to `DefaultTicketCallouts.faq_html` when the card isn't materialized. Content with no `<toggle>` renders unchanged
