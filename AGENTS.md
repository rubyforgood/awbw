...
- `FacilitatorTrainingReport` — Cross-event summary of facilitator trainings grouped by calendar year: scholarship dollars and award counts (funded vs unfunded, via `EventDashboard`) plus trainee counts split into "2-Day" (scheduled) vs "On-Demand" (`event.on_demand?`) totals. Rendered by the shared `events/_facilitator_training_report` partial on the events statistics hub and the standalone `events#facilitator_training_report` page
