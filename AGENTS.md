...
- `OrganizationServices::AutofillChange` — One thing a registrant's answers wrote onto an org: `field`, human `label`, the `value` that landed in it, and an optional `scope` naming which work address. Round-trips through the `form_autofill_changes` JSON column; `#description` renders "Website" or "ZIP on the Austin work address" for a flash, while the linking page shows the value alongside it. `#key` is (field, scope), which is what makes a later submission's value replace an earlier one instead of stacking. Drops a stored entry with no `field` rather than raising — it's a display aid, not a ledger
