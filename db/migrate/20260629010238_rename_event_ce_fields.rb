class RenameEventCeFields < ActiveRecord::Migration[8.1]
  ...
  # gets multiplied out: ce_hours -> ce_hours_available (the decimal hours
  # offered) and ce_hour_cost_cents -> ce_hours_cost_cents (the total CE cost).
  def up
    rename_column :events, :ce_hours, :ce_hours_available if column_exists?(:events, :ce_hours)
    rename_column :events, :ce_hour_cost_cents, :ce_hours_cost_cents if column_exists?(:events, :ce_hour_cost_cents)
  end

  def down
    rename_column :events, :ce_hours_available, :ce_hours if column_exists?(:events, :ce_hours_available)
