  ...
  describe "GET /show" do
    context "with valid token" do
      it "renders successfully" do
        get user_welcome_url(user.invitation_token)
    ...
    context "with expired token" do
      it "redirects with alert" do
        user.update_columns(invitation_created_at: 31.days.ago)
        get user_welcome_url(user.invitation_token)
        expect(response).to redirect_to(root_path)
  ...
  describe "PATCH /update" do
    context "with valid token and password" do
      ...
      it "sets the password" do
        patch user_welcome_update_url(user.invitation_token), params: valid_params
        user.reload
      ...
      it "clears invitation token" do
        ...
        expect(user.invitation_created_at).to be_nil
    ...
    context "with invalid password" do
      ...
      it "renders show with error" do
        patch user_welcome_update_url(user.invitation_token), params: invalid_params
    ...
    context "without password (skip)" do
      it "clears invitation token" do
        patch user_welcome_update_url(user.invitation_token), params: { user: { password: "" } }
        user.reload
        expect(user.invitation_token).to be_nil
      end
