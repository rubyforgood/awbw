...
RSpec.describe Dues::EnsureTerm do
  let(:subscription) { create(:dues_subscription) }

  describe "creating the first term" do
    it "starts on the date asked for and runs a year" do
      term = described_class.call(dues_subscription: subscription, covering: Date.new(2026, 10, 14))
    ...
    it "charges nothing when told to, for a year covered by training" do
      term = described_class.call(dues_subscription: subscription, cost_cents: 0)
  ...
  describe "when a term already covers the date" do
    ...
    it "returns it rather than creating another" do
      expect {
        expect(described_class.call(dues_subscription: subscription, covering: Date.new(2027, 1, 1)))
          .to eq(existing)
      }.not_to change(DuesRegistration, :count)
    end

    it "returns it on the term's final day" do
      expect(described_class.call(dues_subscription: subscription, covering: existing.end_date))
        .to eq(existing)
    end

    it "is idempotent when called twice for the same date" do
      expect {
        2.times { described_class.call(dues_subscription: subscription, covering: Date.new(2027, 1, 1)) }
      }.not_to change(DuesRegistration, :count)
    end
  end
  ...
  describe "renewing" do
    ...
    it "starts the day after the previous term ends" do
      term = described_class.call(dues_subscription: subscription, covering: Date.new(2027, 10, 14))
    ...
    it "creates a term twice running without overlapping" do
      first = described_class.call(dues_subscription: subscription, covering: Date.new(2027, 10, 14))
      second = described_class.call(dues_subscription: subscription, covering: first.end_date + 1.day)

      expect(second.start_date).to eq(first.end_date + 1.day)
      expect(subscription.dues_registrations.count).to eq(3)
    end
  end
  ...
  describe "after a lapse" do
    ...
    it "starts fresh instead of backfilling the missed years" do
      term = described_class.call(dues_subscription: subscription, covering: Date.new(2026, 5, 1))

      expect(term.start_date).to eq(Date.new(2026, 5, 1))
      expect(subscription.dues_registrations.count).to eq(2)
    end
  end
  ...
  describe "a cancelled subscription" do
    ...
    it "creates nothing" do
      expect {
        expect(described_class.call(dues_subscription: subscription)).to be_nil
      }.not_to change(DuesRegistration, :count)
    end

    it "still returns a term that already covers the date" do
      existing = create(:dues_registration, dues_subscription: subscription)

      expect(described_class.call(dues_subscription: subscription)).to eq(existing)
    end
  end
end
