    ...
    link_to person_path(person),
      ...
      avatar = if person.avatar.present?
        image_tag url_for(person.avatar),
                  class: "w-#{size} h-#{size} rounded-full object-cover border border-gray-300 shadow-sm flex-shrink-0"
      elsif person.user&.avatar.present?
        image_tag url_for(person.user.avatar),
                  class: "w-#{size} h-#{size} rounded-full object-cover border border-gray-300 shadow-sm flex-shrink-0"
      else
        image_tag "missing.png",
                  class: "w-#{size} h-#{size} rounded-full object-cover border border-dashed border-gray-300 flex-shrink-0"
      end
