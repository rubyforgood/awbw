class TagsController < ApplicationController
  ...
  def sectors
    ...
    @sectors = authorized_scope(Sector.has_taggings, as: :taggable).order(:name)
  end

  def categories
    authorize! Category, to: :tags_index?
    @categories_by_type = authorized_scope(Category.has_taggings, as: :taggable)
      .joins(:category_type)
  ...
  def load_taggable_sectors_and_categories
    @sectors = authorized_scope(Sector.has_taggings, as: :taggable).order(:name)
    @categories = authorized_scope(Category.has_taggings, as: :taggable)
      .joins(:category_type)
      .select("categories.*, category_types.name AS category_type_name")
      .distinct
