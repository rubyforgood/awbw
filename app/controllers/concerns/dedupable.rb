...
module Dedupable
  ...
  def dedupe_preview
    ...
    @delete_items = @record_to_delete.public_send(config[:join_association]).includes(config[:join_includes])
    @keep_items = @record_to_keep.public_send(config[:join_association]).includes(config[:join_includes])
    @dedupe = build_dedupe_vars(config)
  ...
  def dedupe_execute
    ...
    if respond_to?(:track_event, true)
      track_event("dedupe.#{mn}", {
        ...
        associations_moved: record_to_delete.public_send(config[:join_association]).count
      })
    end

    deduper_class = config[:deduper_class] || "#{mc}Deduper".constantize
    deduper = deduper_class.new(logger: Rails.logger, dry_run: false, min_usage: 0)
  ...
  #   domain:             Symbol for DomainTheme (e.g. :categories)
  #   join_association:   Has-many association name (e.g. :categorizable_items)
  #   join_includes:      Polymorphic includes name (e.g. :categorizable)
  ...
  #   deduper_class:      Deduper service class (optional, defaults to "#{model_class}Deduper")
  def dedupe_config
  ...
  def build_dedupe_vars(config)
    mc = config[:model_class]
    mn = mc.model_name.singular
    join_includes = config[:join_includes]
...

    {
      ...
      item_type_col: "#{join_includes}_type".to_sym,
      item_id_col: "#{join_includes}_id".to_sym,
      join_association: config[:join_association],
