...
# affiliations so the panel shows something to end or reconcile.
...
agreement_forms = [
  ...
  {
    role: "new_job",
    slug: "collab-agreement-new-job",
    name: "Collaboration agreement (new job)",
...
agreement_forms.each do |spec|
  ...
  form.update!(slug: spec[:slug], published: true, role: spec[:role], header: spec[:header], name: spec[:name])
